...
export default function PlayerRatingManager({ bookings }: PlayerRatingManagerProps) {
  ...
  const [viewMode, setViewMode] = useState<'awards' | 'podium'>('awards');
  ...
  // Auto select first eligible booking if none selected
  useEffect(() => {
  ...
  const handleAwardVotesSubmitted = async (votes: Record<string, string>, voterName: string) => {
    if (!selectedBooking) return;

    const newRatings: PlayerRating[] = [];

    Object.entries(votes).forEach(([catId, ratedPlayerIdOrName]) => {
      const playerObj = selectedBooking.players?.find(p => p.id === ratedPlayerIdOrName || p.name === ratedPlayerIdOrName);
      const ratedName = playerObj ? playerObj.name : ratedPlayerIdOrName;

      if (ratedName) {
        newRatings.push({
          id: `rating-${Date.now()}-${catId}`,
          bookingId: selectedBooking.id,
          evaluatorName: voterName,
          ratedPlayerName: ratedName,
          rating: 5
        });
      }
    });
  ...
  return (
    <div className="space-y-6">
      {/* Tab Header with Mode Switching */}
      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-900 tracking-tight flex items-center gap-2">
            <Trophy className="h-6 w-6 text-yellow-500" />
            Melhores do Jogo & Premiação Gamificada
          </h2>
          <p className="text-xs text-slate-500 mt-0.5 font-medium">
            Engaje os atletas com votação rápida pós-partida e acompanhe o Top 3 MVP em tempo real.
          </p>
        </div>

        {/* View mode switcher buttons */}
        <div className="flex flex-wrap sm:flex-nowrap items-center gap-1.5 bg-slate-100 p-1 rounded-xl border border-slate-200/80 w-full sm:w-auto">
          {selectedBooking && (
            <button
              type="button"
              onClick={() => {
                const origin = window.location.origin + window.location.pathname;
                window.open(`${origin}?eval=${selectedBooking.id}`, '_blank');
              }}
              className="px-3.5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs font-black transition flex items-center justify-center gap-1.5 cursor-pointer shadow-xs"
              title="Abrir página de avaliação pública sem login"
            >
              <ExternalLink className="h-4 w-4" />
              <span>Abrir Link de Avaliação</span>
            </button>
          )}
          ...
          >
            ...
            <span>Premiação Gamificada</span>
          </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-slate-400 shrink-0">
                        <Award className={`h-4.5 w-4.5 transition ${isSelected ? 'text-amber-500 scale-110' : ''}`} />
                      </div>
                    </button>
                  );
                })
        ...
        {/* Right Side: Gamified Voting OR Hall of Fame */}
        <div className="lg:col-span-8 space-y-6">
          {selectedBooking ? (
            viewMode === 'awards' ? (
              <PostGameAwards
                selectedBooking={selectedBooking}
                onVoteSubmitted={handleAwardVotesSubmitted}
                onClose={() => setViewMode('podium')}
              />
            ) : (
              <>
                {/* Top 3 Podium Card */}
                  ...
                  {top3.length === 0 ? (
                    <div className="text-center py-12 px-4 flex flex-col items-center justify-center border border-dashed border-slate-200 rounded-2xl">
                      ...
                      <p className="text-[10px] text-slate-400 mt-1 max-w-xs leading-relaxed">
                        Utilize a 'Premiação Gamificada' acima para votar em seus colegas e atualizar o pódio!
                      </p>
                    </div>
                  ) : (
                ...
                <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-4">
                  <h3 className="font-bold text-slate-900 text-xs uppercase tracking-wider flex items-center gap-2">
                    <ThumbsUp className="h-4.5 w-4.5 text-blue-500" /> Atribuir Nota Individual
                  </h3>
            ...
            <div className="bg-white p-12 rounded-2xl border border-slate-200 shadow-sm text-center flex flex-col items-center justify-center min-h-[400px]">
              ...
              <p className="text-xs text-slate-400 mt-1 max-w-xs leading-relaxed">
                Escolha uma partida no menu à esquerda para iniciar a votação pós-jogo.
              </p>
            </div>
          )}
        </div>
